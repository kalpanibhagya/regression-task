...
def read_data() -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    consumption_df = pd.read_csv(os.path.join(directory_path, 'consumptions.csv')).set_index('meter_id')
    temperature_df = pd.read_csv(os.path.join(directory_path, 'temperatures.csv')).set_index('facility_id')
    facility_meter_df = pd.read_csv(os.path.join(directory_path, 'facilities.csv')).set_index('meter_id')
